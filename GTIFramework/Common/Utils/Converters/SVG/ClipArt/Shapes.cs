...
namespace ClipArtViewer
{
	public class Shape : ClipArtElement
	{
        ...
        //lky
        public static Hashtable htTextID = new Hashtable();
	...
	class Group : Shape
	{
        ...
        //lky
        static void htTextIDAdd(List<Shape> list)
        {
            if (list[list.Count - 1].Id.Equals(""))
                return;

            if (list.Count == 0)
                htTextID.Add(list[list.Count - 1].Id, list[list.Count - 1]);
            else if (!htTextID.ContainsKey(list[list.Count - 1].Id))
                htTextID.Add(list[list.Count - 1].Id, list[list.Count - 1]);
        }
