...
{
    public class UC_STPI_PSModel : INotifyPropertyChanged
    {
        ...
        private StndPiDtl fctDtl = new StndPiDtl();
        public StndPiDtl FctDtl
        {
            get{ return fctDtl;}
            set
            {
        ...
        private void OnLoaded(object obj)
        {
            try
            {
                ...
                uC_STPI_PS = obj as UC_STPI_PS;

                btnSave = uC_STPI_PS.btnSave;
        ...
        private void OnSave(object obj)
        {
            ...
            if (!BizUtil.ValidReq(uC_STPI_PS)) return;
            ...
            try
            {
                ...
                BizUtil.Update2(FctDtl, "SaveStndPiDtl");

                //2.위치정보 - 위치편집한 경우만
                if (!FmsUtil.IsNull(GisCmm.WKT_POINT))
                {
                    GisCmm.SavePoint(FctDtl.FTR_CDE, FctDtl.FTR_IDN.ToString(), "WTL_STPI_PS");
                    GisCmm.WKT_POINT = "";
                }
        ...
        private void OnDelete(object obj)
        {
            ...
            try
            {
                BizUtil.Update2(this.FctDtl, "deleteStndPiDtl");
            }
            catch (Exception)
            {
            ...
            // 2.위치정보 삭제처리
            ContentControl cctl = uC_STPI_PS.Parent as ContentControl;
        ...
        private void InitModel()
        {
            ...
            StndPiDtl result = BizUtil.SelectObject(param) as StndPiDtl;
            if (result != null)
            {
                this.FctDtl = result;
            }
            ...
            {
                //신규등록이면 상세화면표시
                if ("Y".Equals(uC_STPI_PS.btnDel.Tag))
                {
                    uC_STPI_PS.grid.Visibility = Visibility.Visible; //DB데이터가 없으면 빈페이지표시
                }
            }
        }
        ...
        private void InitDataBinding()
        {
            ...
            {
                // cbHJD_CDE 행정동
                BizUtil.SetCombo(uC_STPI_PS.cbHJD_CDE, "Select_ADAR_LIST", "HJD_CDE", "HJD_NAM", "선택");

                // cbMNG_CDE 관리기관
                BizUtil.SetCmbCode(uC_STPI_PS.cbMNG_CDE, "250101", "선택");

                // cbSTP_MOP 관재질
                BizUtil.SetCmbCode(uC_STPI_PS.cbSTP_MOP, "250102", "선택");

                // cbVAL_MOF 형식
                BizUtil.SetCmbCode(uC_STPI_PS.cbVAL_MOF, "250035", "선택");
