...
{
    public class UC_BLKM_ASModel : INotifyPropertyChanged
    {
        ...
        private void OnLoaded(object obj)
        {
            try
            {
                ...
                uC_BLKM_AS = obj as UC_BLKM_AS;

                btnSave = uC_BLKM_AS.btnSave;
                ...
                // 콤보변경이벤트설정
                uC_BLKM_AS.cbUPPER_FTR_CDE.SelectedIndexChanged += OnUpFtrCdeChanged;
        ...
        private void OnSave(object obj)
        {
            ...
            if (!BizUtil.ValidReq(uC_BLKM_AS)) return;
            ...
            try
            {
                ...
                BizUtil.Update2(FctDtl, "updateBlk02Dtl");

                //2.위치정보 - 위치편집한 경우만
                if (!FmsUtil.IsNull(GisCmm.WKT_POLYGON))
                {
                    GisCmm.SavePolygon(FctDtl.FTR_CDE, FctDtl.FTR_IDN.ToString(), "WTL_MBLK_AS");
                    GisCmm.WKT_POLYGON = "";
                }
        ...
        private void OnDelete(object obj)
        {
            ...
            try
            {
                BizUtil.Update2(this.FctDtl, "deleteBlk02Dtl");
            }
            catch (Exception)
            {
            ...
            // 2.위치정보 삭제처리
            ContentControl cctl = uC_BLKM_AS.Parent as ContentControl;
        ...
        private void InitModel()
        {
            ...
            {
                //신규등록이면 상세화면표시
                if ("Y".Equals(uC_BLKM_AS.btnDel.Tag))
                {
                    uC_BLKM_AS.grid.Visibility = Visibility.Visible; //DB데이터가 없으면 빈페이지표시
                }
            }
        }
