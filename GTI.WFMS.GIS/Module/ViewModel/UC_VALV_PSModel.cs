    ...
    public class UC_VALV_PSModel : INotifyPropertyChanged
    {
        ...
        private void InitDataBinding()
        {
            try
            {
                ...
                // cbMNG_CDE 관리기관
                BizUtil.SetCmbCode(uC_VALV_PS.cbMNG_CDE, "250101", true);
                // cbVAL_MOF 형식
                BizUtil.SetCmbCode(uC_VALV_PS.cbVAL_MOF, "250016", true);

                // cbVAL_MOP 관재질
                BizUtil.SetCmbCode(uC_VALV_PS.cbVAL_MOP, "250015", true);

                // cbSAE_CDE    제수변회전방향
                BizUtil.SetCmbCode(uC_VALV_PS.cbSAE_CDE, "250027", true);

                // cbMTH_CDE    제수변구동방법
                BizUtil.SetCmbCode(uC_VALV_PS.cbMTH_CDE, "250065", true);

                // cbVAL_FOR    시설물형태(=구조물형태)
                BizUtil.SetCmbCode(uC_VALV_PS.cbVAL_FOR, "250007", true);

                // cbCST_CDE    이상상태
                BizUtil.SetCmbCode(uC_VALV_PS.cbCST_CDE, "250062", true);

                // cbOFF_CDE    개폐여부
                BizUtil.SetCmbCode(uC_VALV_PS.cbOFF_CDE, "250036", true);
            }
            catch (Exception ex)
            {
