...
namespace GTI.WFMS.GIS.Module.ViewModel
{
    public class UC_VALV_PSModel : INotifyPropertyChanged
    {
        ...
        private void InitDataBinding()
        {
            ...
            {
                // cbHJD_CDE 행정동
                BizUtil.SetCombo(uC_VALV_PS.cbHJD_CDE, "Select_ADAR_LIST", "HJD_CDE", "HJD_NAM", true);

                // cbMNG_CDE 관리기관
                BizUtil.SetCmbCode(uC_VALV_PS.cbMNG_CDE, "MNG_CDE", true);
                // cbVAL_MOF 형식
                BizUtil.SetCmbCode(uC_VALV_PS.cbVAL_MOF, "MOF_CDE", true, "250016");

                // cbVAL_MOP 관재질
                BizUtil.SetCmbCode(uC_VALV_PS.cbVAL_MOP, "MOP_CDE", true, "250015");

                // cbSAE_CDE    제수변회전방향
                BizUtil.SetCmbCode(uC_VALV_PS.cbSAE_CDE, "SAE_CDE", true);

                // cbMTH_CDE    제수변구동방법
                BizUtil.SetCmbCode(uC_VALV_PS.cbMTH_CDE, "MTH_CDE", true);

                // cbVAL_FOR    시설물형태(=구조물형태)
                BizUtil.SetCmbCode(uC_VALV_PS.cbVAL_FOR, "FOR_CDE", true);

                // cbCST_CDE    이상상태
                BizUtil.SetCmbCode(uC_VALV_PS.cbCST_CDE, "CST_CDE", true);

                // cbOFF_CDE    개폐여부
                BizUtil.SetCmbCode(uC_VALV_PS.cbOFF_CDE, "OFF_CDE", true);
            }
            catch (Exception ex)
            {
