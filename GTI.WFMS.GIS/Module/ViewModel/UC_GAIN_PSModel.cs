...
{
    public class UC_GAIN_PSModel : INotifyPropertyChanged
    {
        ...
        private IntkStDtl fctDtl = new IntkStDtl();
        public IntkStDtl FctDtl
        {
            get{ return fctDtl;}
            set
            {
        ...
        private void OnLoaded(object obj)
        {
            try
            {
                ...
                uC_GAIN_PS = obj as UC_GAIN_PS;

                btnSave = uC_GAIN_PS.btnSave;
        ...
        private void OnSave(object obj)
        {
            ...
            if (!BizUtil.ValidReq(uC_GAIN_PS)) return;
        ...
        private void OnDelete(object obj)
        {
            ...
            try
            {
                BizUtil.Update2(this.FctDtl, "deleteIntkStDtl");
            }
            catch (Exception)
            {
            ...
            // 2.위치정보 삭제처리
            ContentControl cctl = uC_GAIN_PS.Parent as ContentControl;
        ...
        private void InitModel()
        {
            ...
            IntkStDtl result = BizUtil.SelectObject(param) as IntkStDtl;
            if (result != null)
            {
                this.FctDtl = result;
            }
            ...
            {
                //신규등록이면 상세화면표시
                if ("Y".Equals(uC_GAIN_PS.btnDel.Tag))
                {
                    uC_GAIN_PS.grid.Visibility = Visibility.Visible; //DB데이터가 없으면 빈페이지표시
                }
            }
        }
        ...
        private void InitDataBinding()
        {
            ...
            {
                // cbHJD_CDE 행정동
                BizUtil.SetCombo(uC_GAIN_PS.cbHJD_CDE, "Select_ADAR_LIST", "HJD_CDE", "HJD_NAM", "선택");

                // cbMNG_CDE 관리기관
                BizUtil.SetCmbCode(uC_GAIN_PS.cbMNG_CDE, "250101", "선택");

                // cbWSR_CDE 수원구분
                BizUtil.SetCmbCode(uC_GAIN_PS.cbWSR_CDE, "250058", "선택");

                // cbWRW_CDE 도수방법
                BizUtil.SetCmbCode(uC_GAIN_PS.cbWRW_CDE, "250046", "선택");

                // cbWRW_CDEE 취소방법
                BizUtil.SetCmbCode(uC_GAIN_PS.cbWGW_CDE, "250069", "선택");
            }
            catch (Exception ex)
            {
