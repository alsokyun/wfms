...
namespace GTI.WFMS.GIS.Module.ViewModel
{
    public class UC_RSRV_PSModel : INotifyPropertyChanged
    {
        ...
        private void InitDataBinding()
        {
            ...
            {
                // cbHJD_CDE 행정동
                BizUtil.SetCombo(uC_RSRV_PS.cbHJD_CDE, "Select_ADAR_LIST", "HJD_CDE", "HJD_NAM", true);

                // cbMNG_CDE 관리기관
                BizUtil.SetCmbCode(uC_RSRV_PS.cbMNG_CDE, "MNG_CDE", true);
                
                // cbBLS_CDE 건물유형
                BizUtil.SetCmbCode(uC_RSRV_PS.cbBLS_CDE, "BLS_CDE", true);
            }
            catch (Exception ex)
            {
