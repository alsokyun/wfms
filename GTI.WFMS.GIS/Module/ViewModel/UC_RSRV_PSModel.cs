...
{
    public class UC_RSRV_PSModel : INotifyPropertyChanged
    {
        ...
        private WtrTrkDtl fctDtl = new WtrTrkDtl();
        public WtrTrkDtl FctDtl
        {
            get{ return fctDtl;}
            set
            {
        ...
        private void OnLoaded(object obj)
        {
            try
            {
                ...
                uC_RSRV_PS = obj as UC_RSRV_PS;

                btnSave = uC_RSRV_PS.btnSave;
        ...
        private void OnSave(object obj)
        {
            ...
            if (!BizUtil.ValidReq(uC_RSRV_PS)) return;
            ...
            try
            {
                ...
                BizUtil.Update2(FctDtl, "SaveWtrTrkDtl");

                //2.위치정보 - 위치편집한 경우만
                if (!FmsUtil.IsNull(GisCmm.WKT_POINT))
                {
                    GisCmm.SavePoint(FctDtl.FTR_CDE, FctDtl.FTR_IDN.ToString(), "WTL_RSRV_PS");
                    GisCmm.WKT_POINT = "";
                }
        ...
        private void InitModel()
        {
            ...
            WtrTrkDtl result = BizUtil.SelectObject(param) as WtrTrkDtl;
            if (result != null)
            {
                this.FctDtl = result;
            }
            else
            {
                ...
                {
                    uC_RSRV_PS.grid.Visibility = Visibility.Visible; //DB데이터가 없으면 빈페이지표시
                }
            }
        }
        ...
        private void InitDataBinding()
        {
            ...
            {
                // cbHJD_CDE 행정동
                BizUtil.SetCombo(uC_RSRV_PS.cbHJD_CDE, "Select_ADAR_LIST", "HJD_CDE", "HJD_NAM", "선택");

                // cbMNG_CDE 관리기관
                BizUtil.SetCmbCode(uC_RSRV_PS.cbMNG_CDE, "250101", "선택");
                
                // cbBLS_CDE 건물유형
                BizUtil.SetCmbCode(uC_RSRV_PS.cbBLS_CDE, "250037", "선택");
            }
            catch (Exception ex)
            {
